...
if (prohibited.some(word => m.text.toLowerCase().includes(word))) return m.reply('Eso esta prohibido')      
try {
const res = await googleImage(text)
let image = res.getRandom()
let link = image
conn.sendButton(m.chat, `${mssg.aquiImagen}`, wm, link, [[mssg.botonA, `/imagen ${text}`]], null, null, m)

} catch (e) {
console.log(`${mssg.menError}`)
console.log(e)
handler.money = false
}}
