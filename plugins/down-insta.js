...
if (!args[0]) throw `${mssg.eInstagram}`
...
const tXXxt = `${mssg.aquiVideo}`.trim();
...
const txt2 = `${mssg.aquiVideo}`.trim();
...
const txt4 = `${mssg.aquiVideo}`.trim();
...
const txt1 = `${mssg.aquiVideo}`.trim();
await conn.sendFile(m.chat, videoig, 'error.mp4', txt1, m);
await conn.sendMessage(m.chat, {text: waittttt, edit: key})
} catch (e) {
conn.sendMessage(m.chat, {text: `${mssg.menError}`, edit: key});
