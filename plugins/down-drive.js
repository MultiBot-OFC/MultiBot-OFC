...
if (!args[0]) throw `${mssg.eDrive}`
let url=args[0]
if (!(url && url.match(/drive\.google/i))) throw `${mssg.noValido} Drive`
let res = await fdrivedl(url)
let caption=`    
${mssg.playBordeA}
${mssg.playBordec}${mssg.Nombre}
${mssg.playBorded}${res.fileName}

${mssg.playBordec}${mssg.Peso}
${mssg.playBorded}${formatBytes(res.sizeBytes)}

${mssg.playBordec}${mssg.Paquete}
${mssg.playBorded}${res.mimetype}
${mssg.playBordeB}`.trim()
m.reply(`${caption}`)
let fileSize=formatBytes(res.sizeBytes)
if (fileSize.includes('GB') && parseInt(fileSize.replace(' GB', '')) > 1.8) throw `${mssg.pesoAlto}`
conn.sendMessage(m.chat, { document: { url: res.downloadUrl }, fileName: res.fileName, mimetype: res.mimetype }, { quoted: m })
}
async function fdrivedl(url) {
      let id;
      id = (url.match(/\/?id=(.+)/i) || url.match(/\/d\/(.*?)\//))[1];
      if (!id) throw `No se encontraron resultados...`;
      let res = await fetch(
